...
>(
    (
        ...
        const adjustDropdownPosition = useCallback(() => {
            if (dropdownRef.current) {
                ...
                if (spaceBelow >= dropdownHeight) {
                    ...
                    setSelHeight(prev => prev !== defaultHeight ? defaultHeight : prev);
                    ...
                    if (itemsRef?.current) {
                        ...
                        const newHeight = selH < 50 ? 50 : selH;
                        setSelHeight(prev => prev !== newHeight ? newHeight : prev);
                    }
                }
                // setDropdownStyle({
                //   ...dropdownPosition,
        ...
        }, [viewMode, countOnly, tab.length, tabInlineSearch, filteredData?.length, enableSelectAll, isMultiple, topMargin, showAllSelected]);
        ...
        useEffect(() => {
            ...
            // eslint-disable-next-line react-hooks/exhaustive-deps
        }, [selectedItems, adjustDropdownPosition]);
                                    ...
                                    <div key={item.id} className="selected-item">
                                        ...
                                        {!viewMode && (
                                            <button
                                                ...
                                                aria-label={`Remove ${String(item?.[desc])}`}
                                            >
                                                X
                                            </button>
                                        )}
        ...
        // Auto-refresh main dropdown data when tabSelectedItems change (add/remove)
        useEffect(() => {
            if (autoRefreshMainDropdown && dropOpen && async) {
        ...
        }, [autoRefreshMainDropdown, dropOpen, tabSelectedItems, activeTab, tab, tabInlineSearch, searchValue, async, handlePickSuggestions]);
                ...
                >
                    ...
                    {enableToolsTab && (
                        <DropdownFilterTabs
                            ...
                            onChange={onToolTabChange}
                            countOnly={true}
                            tab={toolTabs}
                            clearTabSwitch={toolTabClearSwitch}
                            tabInlineSearch={true}
...
export default React.memo(AutoCompleteWithTabFilter);
