...
const ModernAutoComplete: React.FC<AutoSuggestionInputProps> = ({
  ...
  const handleScroll = (event: any) => {
    ...
    // Ignore scrolls inside the main autocomplete container or dropdown content
    if (
      scrollTarget &&
  ...
  const setDropDown = () => {
    return (
      (filteredData?.length > 0 || showNoResults) && (
        ...
        >
          <ul
            className="qbs-h-auto qbs-max-h-40 qbs-overflow-auto qbs-w-full qbs-py-1.5"
          ...
          >
            {filteredData?.length > 0 ? (
              ...
              !isLoading && (
                <li
                  className="qbs-autocomplete-notfound qbs-cursor-default qbs-p-1 qbs-rounded-sm qbs-text-xxs"
                  aria-disabled="true"
                >
                  No Results Found
                </li>
