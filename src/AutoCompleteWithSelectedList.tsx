    ...
    useEffect(() => {
      ...
      const handleClickOutside = (event: any) => {
        ...
        if (
          ...
          (event.target as HTMLElement)?.id !== uniqueDropArrowId
        ) {
          setTimeout(() => {
            setDropOpen(false);
            //if (!typeOnlyFetch) setSearchValue('');
          ...
          {(selectedItems?.length > 0 || !tabInlineSearch) && (
            <>
              {!countOnly ? (
                ...
                <div className={`selected-items-counter-container selected-item-inline-counter ${disabled ? 'selected-item-inline-counter-disabled' : ''} qbs-text-sm qbs-gap-1 ${!tabInlineSearch && autoDropdown && !disabled ? 'selected-item-nontool-direct-counter' : ''}`}>
